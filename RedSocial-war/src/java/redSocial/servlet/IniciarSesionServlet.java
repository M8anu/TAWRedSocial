...
public class IniciarSesionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (email.equals("") || pass.equals("")){
            String mensajeError = "Email o contraseña vacio";
            request.setAttribute("mensaje", mensajeError);
            RequestDispatcher rd = this.getServletContext().getRequestDispatcher("/iniciarSesionIncorrecto.jsp");
            rd.forward(request, response);
        }else{
            Usuario us = usuarioFacade.findByEmail(email);
            if (us.getEmail().equals(email) && us.getPassword().equals(pass)){
                request.setAttribute("usuario", us);
                RequestDispatcher rd = this.getServletContext().getRequestDispatcher("/principal.jsp");
                rd.forward(request, response); 
            }else{
